...
namespace
{
	...
	ID3D11Buffer* g_pVSConstantBufferProj = nullptr;
	ID3D11Buffer* g_pVSConstantBufferWorld = nullptr;
	ID3D11PixelShader* g_pPixelShader = nullptr;
	ID3D11SamplerState* g_pSamplerState = nullptr;
	ID3D11Buffer* g_pPSConstantBuffer = nullptr;
...
bool Shader2D_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (!pDevice || !pContext) {
		hal::dout << "Shader2D_Initialize() : 与えられたデバイスかコンテキストが不正です" << std::endl;
		return false;
	}
	...
	g_pDevice->CreateBuffer(&buffer_desc, nullptr, &g_pVSConstantBufferProj);
	g_pDevice->CreateBuffer(&buffer_desc, nullptr, &g_pVSConstantBufferWorld);
	...
	//sampler_desc.Filter = D3D11_FILTER_ANISOTROPIC;		//3D
	sampler_desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;	//2D
...
void Shader2D_SetProjMatrix(const DirectX::XMMATRIX& matrix)
{
	...
	// 定数バッファに行列をセット
	g_pContext->UpdateSubresource(g_pVSConstantBufferProj, 0, nullptr, &transpose, 0, 0);
}
...
void Shader2D_SetWorldMatrix(const DirectX::XMMATRIX& matrix)
{
	...
	// 定数バッファに行列をセット
	g_pContext->UpdateSubresource(g_pVSConstantBufferWorld, 0, nullptr, &transpose, 0, 0);
}
...
void Shader2D_Begin()
{
	...
	// 定数バッファを描画パイプラインに設定
	g_pContext->VSSetConstantBuffers(0, 1, &g_pVSConstantBufferProj);
	g_pContext->VSSetConstantBuffers(1, 1, &g_pVSConstantBufferWorld);
